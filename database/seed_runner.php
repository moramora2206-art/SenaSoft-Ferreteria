...
echo "Conexión a MySQL exitosa ($db).\n";

// 1. INSERCIÓN DE CLIENTES DE FERRETERÍA (Id 10-13)
$clientesSQL = "INSERT IGNORE INTO `clientes` (`IdCliente`, `Nombre`, `Apellido`, `Cédula`, `NCelular`, `Email`, `Dirección`) VALUES
...
if ($conn->query($clientesSQL)) {
    echo "✔ Clientes de prueba insertados/verificados (" . $conn->affected_rows . " filas afectadas).\n";
} else {
    echo "❌ Error en clientes: " . $conn->error . "\n";
}

// 2. INSERCIÓN DE PRODUCTOS DE FERRETERÍA (Id 10-21)
$productosSQL = "INSERT IGNORE INTO `productos` (`IdProducto`, `IdProveedor`, `Nombre_Producto`, `Codigo_SKU`, `Stock`, `Precio_Unitario`, `Precio_Compra`, `Categoria`, `Imagen`, `Fecha_Vencimiento`, `Descripcion`) VALUES
...
if ($conn->query($productosSQL)) {
    echo "✔ Productos de prueba insertados/verificados (" . $conn->affected_rows . " filas afectadas).\n";
} else {
    echo "❌ Error en productos: " . $conn->error . "\n";
}

$conn->close();
echo "Proceso de inserción completado con éxito.\n";
