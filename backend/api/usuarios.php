 ...
 * Cualquier usuario autenticado sin el rol recibe 403 FORBIDDEN.
 */
...
switch ($method) {
    case 'GET':
        if (isset($_GET['id']) && !empty($_GET['id'])) {
            ...
            $code = $res["success"] ? 200 : 404;
            jsonResponse(
                $res["success"],
                $res["message"] ?? ($res["success"] ? "" : "Usuario no encontrado."),
                $res["data"] ?? null,
                $code,
                $res["success"] ? [] : ["errorCode" => "NOT_FOUND"]
            );
        } else {
            $res = $controller->listar();
            jsonResponse($res["success"], "Lista de usuarios", $res["data"]);
        }
        break;
